...
wget.download(link, out=tmp_path + 'nasdaqlisted.txt')
nsdq_full_df = pd.read_csv(tmp_path + 'nasdaqlisted.txt', sep='|', header=0)
...
def handler(event, context):
    ...
    message += "\nRATIOS" + sep + """Ratios can be interpreted as percentages ranging from -99.99 representing a total loss of value, to 0.00 which represents that a stock is at its high point for the period selected.\n """

    daily_nsdq = read_tickers(mode="NASDAQ", period="2d")[:25]
    ...
    weekly_nsdq = read_tickers(mode="NASDAQ", period="7d")[:25]
    message += create_message(weekly_nsdq, mode='NASDAQ', period="weekly")

    weekly_snp = read_tickers(mode="S&P", period="7d")[:25]
    message += create_message(weekly_snp, mode='S&P', period="weekly")

    monthly_nsdq = read_tickers(mode="NASDAQ", period="1mo")[:25]
